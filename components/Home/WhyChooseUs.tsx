...
export default function WhyChooseUs() {
  ...
  // Auto carousel effect every 3 seconds
  useEffect(() => {
        ...
        <div className="relative w-full max-w-[90%] sm:max-w-[460px] md:max-w-[540px] h-[260px] sm:h-[360px] md:h-[460px] flex items-center justify-center mx-auto">
          {slides.map((slide, i) => {
            ...
            return (
              <div
  key={i}
  className="absolute flex items-center justify-center overflow-hidden transition-all duration-500 ease-in-out shadow-xl"
  style={{
    width: "530px", // fixed width
    height: "485px", // fixed height
...
>
  {slide.type === "image" ? (
    <Image
      ...
      alt={`Slide ${i + 1}`}
      fill
      className="object-cover rounded-[36px]" // inner rounding
    />
  ) : (
              ...
              <span
                ...
                style={
                  i === current
                    ? {
                        background:
                          "linear-gradient(90deg, #B74254 0%)",
                      }
                    : {
                        background:
